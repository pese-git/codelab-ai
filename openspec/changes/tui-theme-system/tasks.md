...
- [ ] 6.1 Проверить все 57 компонентов с DEFAULT_CSS на использование hex цветов
- [ ] 6.2 Заменить hex цвета на Textual CSS variables (`$primary`, `$surface`, `$text-muted`) в компонентах
- [ ] 6.3 Проверить что `message_bubble.py` адаптируется к теме
- [ ] 6.4 Проверить что `action_button.py` адаптируется к теме
- [x] 6.5 Проверить что `markdown.py` адаптируется к теме (включая CodeBlock theme)
- [ ] 6.6 Проверить что `quick_actions_bar.py` адаптируется к теме
- [ ] 6.7 Проверить что `footer_bar.py` адаптируется к теме
